...
**Status:** v1 landed. `hats verify [<account>|--all]` complements doctor's layout checks with deep per-account token semantics: JSON well-formedness, credential file mode (600/400), provider-specific parse (claude `claudeAiOauth` horizon + refreshToken presence + RC-scope; codex `auth_mode` sanity, `id_token` JWT horizon, refresh-token freshness, `cli_auth_credentials_store=file`, and `codex login status` liveness). WARN/FAIL discipline: expired-with-refresh is WARN (auto-recovers); expired-without-refresh is FAIL (operator must re-login); non-JSON is FAIL. Exit 0 clean / 1 any issue. Read-only; never mutates state. Smoke `test_verify_command` covers happy path + expired-with-refresh WARN + non-JSON FAIL + unknown flag + missing account; codex coverage lives in `test_codex_verify_command`. Symmetry audit picked up 22 checks (was 21) — verify adds a provider-agnostic surface probe.
